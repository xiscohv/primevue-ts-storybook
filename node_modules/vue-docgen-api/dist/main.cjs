...
function resolveIdentifier(ast, path) {
    ...
    recast.visit(ast.program, {
        ...
        visitVariableDeclaration: function (variablePath) {
            if (!bt__namespace.isVariableDeclaration(variablePath.node)) {
                return false;
            }
            var varID = variablePath.node.declarations[0].id;
            if (!varID || !bt__namespace.isIdentifier(varID) || varID.name !== varName) {
                return false;
            }
        ...
        visitClassDeclaration: function (classPath) {
            ...
            if (!classID || !bt__namespace.isIdentifier(classID) || classID.name !== varName) {
                return false;
            }
            comp = classPath;
            return false;
    ...
    recast.visit(ast.program, {
        ...
        visitExportDefaultDeclaration: function (astPath) {
            if (variableFilter.indexOf('default') > -1) {
                ...
                if (bt__namespace.isIdentifier(middleNameDeclaration)) {
...
function getTypeObjectFromTSType(type) {
    return bt__namespace.isTSUnionType(type) || bt__namespace.isTSIntersectionType(type)
        ? { name: TS_TYPE_NAME_MAP[type.type], elements: type.types.map(getTypeObjectFromTSType) }
        : bt__namespace.isTSArrayType(type)
                                ...
                                case 1:
                                    ...
                                    recast.visit(ast, {
                                        visitExportNamedDeclaration: function (p) {
                                            ...
                                            if (masterDeclaration && bt__namespace.isVariableDeclaration(masterDeclaration)) {
                                                masterDeclaration.declarations.forEach(function (declaration, i) {
                                                    if (bt__namespace.isVariableDeclarator(declaration) &&
                                                        bt__namespace.isIdentifier(declaration.id) &&
                            ...
                            astTypes.visit(ast, {
                                visitVariableDeclaration: function (p) {
                                    p.node.declarations.forEach(function (decl, i) {
                                        if (bt__namespace.isVariableDeclarator(decl) &&
                                            bt__namespace.isIdentifier(decl.id) &&
...
function eventHandler$1(documentation, path, astPath) {
    ...
    recast.visit(path.node, {
        visitCallExpression: function (pathExpression) {
            if (bt__namespace.isMemberExpression(pathExpression.node.callee) &&
                bt__namespace.isThisExpression(pathExpression.node.callee.object) &&
                bt__namespace.isIdentifier(pathExpression.node.callee.property) &&
        ...
        recast.visit(path.node, {
            visitClassMethod: function (nodePath) {
                if (nodePath.node.decorators &&
                    bt__namespace.isCallExpression(nodePath.node.decorators[0].expression) &&
                    bt__namespace.isIdentifier(nodePath.node.decorators[0].expression.callee) &&
                    ...
                    else if (bt__namespace.isIdentifier(nodePath.node.key)) {
    ...
    if (bt__namespace.isObjectExpression(path.node)) {
        ...
        recast.visit(renderValuePath.node, {
            ...
            visitCallExpression: function (pathCall) {
                if (bt__namespace.isMemberExpression(pathCall.node.callee) &&
                    bt__namespace.isMemberExpression(pathCall.node.callee.object) &&
                    bt__namespace.isThisExpression(pathCall.node.callee.object.object) &&
                    bt__namespace.isIdentifier(pathCall.node.callee.property) &&
                    bt__namespace.isIdentifier(pathCall.node.callee.object.property) &&
                    ...
                    if (bt__namespace.isObjectExpression(bindings) && bindings.properties.length) {
            ...
            visitMemberExpression: function (pathMember) {
                if (bt__namespace.isMemberExpression(pathMember.node.object) &&
                    bt__namespace.isThisExpression(pathMember.node.object.object) &&
                    bt__namespace.isIdentifier(pathMember.node.object.property) &&
                    (pathMember.node.object.property.name === '$slots' ||
                        pathMember.node.object.property.name === '$scopedSlots') &&
                    bt__namespace.isIdentifier(pathMember.node.property)) {
            ...
            visitJSXElement: function (pathJSX) {
                ...
                if (bt__namespace.isJSXIdentifier(tagName) && tagName.name === 'slot') {
        ...
        if (contextVariable.value) {
            if (bt__namespace.isIdentifier(contextVariable.value)) {
                ...
                recast.visit(renderValuePath.node, {
                    // context.children
                    visitMemberExpression: function (pathMember) {
                        if (bt__namespace.isIdentifier(pathMember.node.object) &&
...
function getPropsFromExpression(documentation, item, exp, siblings) {
    ...
    recast.visit(ast.program, {
        visitMemberExpression: function (path) {
            ...
            if (obj &&
                propName &&
                bt__namespace.isIdentifier(obj) &&
                obj.name === 'props' &&
                bt__namespace.isIdentifier(propName)) {
...
function getEventsFromExpression(item, expression, documentation, siblings) {
    ...
    recast.visit(ast.program, {
        visitCallExpression: function (path) {
            ...
            if (obj && args && bt__namespace.isIdentifier(obj) && obj.name === '$emit' && args.length) {
                var evtName = bt__namespace.isStringLiteral(args[0]) ? args[0].value : '<undefined>';
...
function getTypeDefinitionFromIdentifier(astPath, typeName, opt) {
    ...
    recast.visit(astPath.program, {
        ...
        visitTSInterfaceDeclaration: function (nodePath) {
            if (bt__namespace.isIdentifier(nodePath.node.id) && nodePath.node.id.name === typeName) {
                ...
                if (nodePath.value.extends) {
                    ...
                    parentInterfaces.forEach(function (parentInterface) {
                        if (!bt__namespace.isIdentifier(parentInterface.expression)) {
                            return;
                        }
                        ...
                        parentInterfaceBody === null || parentInterfaceBody === void 0 ? void 0 : parentInterfaceBody.value.forEach(function (parentInterfaceProp) {
                            if (!interfaceBody_1.value.find(function (prop) {
                                return bt__namespace.isIdentifier(prop.key) &&
                                    bt__namespace.isIdentifier(parentInterfaceProp.key) &&
        ...
        return __generator(this, function (_a) {
            recast.visit(astPath.program, {
                visitCallExpression: function (nodePath) {
                    if (bt__namespace.isIdentifier(nodePath.node.callee) && nodePath.node.callee.name === 'defineEmits') {
...
function setupOptionsHandler(documentation, componentDefinition, astPath, opt) {
    return __awaiter(this, void 0, void 0, function () {
        return __generator(this, function (_a) {
            recast.visit(astPath.program, {
                visitCallExpression: function (nodePath) {
                    if (bt__namespace.isIdentifier(nodePath.node.callee) &&
                        nodePath.node.callee.name === 'defineOptions' &&
                        bt__namespace.isObjectExpression(nodePath.node.arguments[0])) {
                        var options = nodePath.node.arguments[0];
                        options.properties.forEach(function (property) {
                            if (bt__namespace.isObjectProperty(property) && bt__namespace.isIdentifier(property.key)) {
                                var key = property.key.name;
                                if (key === 'name' && bt__namespace.isStringLiteral(property.value)) {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    recast.visit(astPath.program, {
                        visitCallExpression: function (nodePath) {
                            var hasDefaults = bt__namespace.isIdentifier(nodePath.node.callee) && nodePath.node.callee.name === 'withDefaults';
...
function setupExposedHandler(documentation, componentDefinition, astPath, opt) {
    return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            recast.visit(astPath.program, {
                visitCallExpression: function (nodePath) {
                    if (bt__namespace.isIdentifier(nodePath.node.callee) && nodePath.node.callee.name === 'defineExpose') {
                        if (bt__namespace.isObjectExpression(nodePath.get('arguments', 0).node)) {
...
var setupHandlers = [setupEventHandler, setupOptionsHandler, setupPropHandler, setupExposedHandler];
